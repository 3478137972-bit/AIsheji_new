...
- text_content: 文案内容（主标题、标语、产品名称、促销信息等）**【重要】**
- text_style: 文字排版风格（大胆醒目、优雅细腻、简洁有力等）
- aspect_ratio: 图片比例（1:1, 2:3, 3:2, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9, 21:9, auto）**【重要】**

注意：
1. 只询问对设计至关重要的信息
2. 不要询问过于细节的信息
3. **对于海报、广告等需要文字的设计，必须询问文案内容（text_content）**
4. **必须询问用户想要的图片比例（aspect_ratio）**，提供的选项必须严格基于支持的比例列表（1:1, 2:3, 3:2, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9, 21:9, auto），最后一个选项是"auto (自动)"，不要添加"其他自定义比例"
5. 如果用户已经提供了足够信息，isComplete应为true`;
