...
2. **只询问对设计至关重要且无法推断的信息**
3. 对于 Logo 设计：如果用户提供了品牌名称和行业信息，通常已经足够，可以直接生成
4. 对于海报、广告等需要文字的设计：如果缺少具体文案内容（text_content），才需要询问
5. 图片比例（aspect_ratio）：如果用户没有明确要求，使用 "auto" 即可，不需要询问
6. 设计风格（style）：如果用户没有明确要求，可以根据品牌/行业推断，不需要询问
7. **如果用户的需求已经包含了核心信息（如品牌名称、设计类别、基本描述），应该认为 isComplete=true**

**什么时候 isComplete 应该为 true**：
- Logo 设计：用户提供了品牌名称或基本描述
- 插画设计：用户提供了主题或场景描述
- 海报设计：用户提供了主题（文案可以在生成时智能补充）
- 包装设计：用户提供了产品信息
- IP 形象：用户提供了角色描述或特征

**什么时候才需要询问**：
- 用户的需求过于模糊（如只说"我想做设计"）
- 缺少核心信息且无法推断（如海报设计但完全没有主题）
- 用户明确表示需要帮助选择（如"帮我选个风格"）`;
