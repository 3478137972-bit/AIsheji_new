...
你的任务是分析用户的设计需求是否完整，识别缺少哪些**必须**的关键信息。
...
## Logo 设计必须信息
- **style**: 设计风格（现代简约、复古经典、科技感、手绘风、可爱卡通、高端奢华等）
- **color_preference**: 色调偏好（冷色调、暖色调、高对比、柔和、多彩、单色等）
- **aspect_ratio**: 图片比例

## 海报设计必须信息
- **text_content**: 文案内容（主标题、副标题、宣传语等具体文字）
- **style**: 设计风格
- **color_preference**: 色调偏好
- **aspect_ratio**: 图片比例

## 插画设计必须信息
- **style**: 设计风格（扁平插画、手绘风、水彩风、矢量风等）
- **color_preference**: 色调偏好
- **aspect_ratio**: 图片比例

## 包装设计必须信息
- **product_info**: 产品信息（产品名称、类型、特点）
- **style**: 设计风格
- **color_preference**: 色调偏好
- **aspect_ratio**: 图片比例

## IP形象设计必须信息
- **character_description**: 角色描述（外观、性格、特征）
- **style**: 设计风格
- **color_preference**: 色调偏好
- **aspect_ratio**: 图片比例
...
**什么时候 isComplete = true**：
- 用户已经提供了该设计类型的**所有必须信息**
- 例如 Logo 设计：用户提供了 style + color_preference + aspect_ratio

**什么时候 isComplete = false**：
- 缺少任何一个必须信息
- 必须在 missingInfo 中列出**所有**缺失的必须信息

# 询问策略

1. **一次性询问所有缺失的必须信息**
   - 不要分批询问，避免多轮对话
   - 将所有缺失的必须信息放在一个 missingInfo 数组中

2. **优先使用用户已提供的信息**
   - 不要重复询问用户已经说过的内容

3. **提供选项帮助用户选择**
   - 对于 style、color_preference、aspect_ratio 等，提供具体选项
   - 选项要丰富且实用，包含"其他（请描述）"选项

4. **问题要清晰具体**
   - 不要问"您想要什么风格"，而是"您希望这个LOGO呈现什么样的设计风格？"
   - 提供示例帮助用户理解
...
分析：
- 已提供：品牌类型（烘焙）
- 缺失：style（设计风格）、color_preference（色调偏好）、aspect_ratio（图片比例）
- isComplete: false
- missingInfo: [style, color_preference, aspect_ratio] 三个问题一起问`;

    const userPrompt = `用户需求：${userInput}

请分析这个需求是否完整，缺少哪些**必须**的关键信息。记住：必须一次性列出所有缺失的必须信息。`;
...
4. **问题要清晰具体**
   ...
   - 提供示例帮助用户理解`;
    ...
    const userPrompt = `基于我们之前的对话，请分析这个需求是否完整，缺少哪些**必须**的关键信息。记住：必须一次性列出所有缺失的必须信息。`;
