...
async function getHistory(options = {}) {
  const { limit = 20, offset = 0, tool_name } = options;
  console.log(`[HistoryStore] 查询历史记录 (limit: ${limit}, offset: ${offset}, tool: ${tool_name || 'all'})`);
...
async function clearHistory(options = {}) {
  ...
  console.log(`[HistoryStore] 清空历史记录 (olderThan: ${olderThan || 'all'})`);
    ...
    if (client) {
      ...
      if (olderThan) {
        ...
        memoryStore.splice(0, memoryStore.length,
          ...memoryStore.filter(item => new Date(item.created_at) >= olderThan)
        );
