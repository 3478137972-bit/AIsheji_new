...
async function getIllustrationTaskStatus(taskId) {
  try {
    ...
    if (response.data.code === 200) {
      ...
      // 调试日志:输出KIEAI原始响应
      console.log('【调试】KIEAI原始响应 taskId:', data.taskId);
      console.log('【调试】state:', data.state);
      console.log('【调试】resultJson:', data.resultJson);
      ...
      if (data.state === 'success' && data.resultJson) {
        try {
          ...
          console.log('【调试】解析后的result:', JSON.stringify(result, null, 2));
          if (result.resultUrls && result.resultUrls.length > 0) {
            imageUrl = result.resultUrls[0];
            status = 'success';
            console.log('【调试】提取的imageUrl:', imageUrl);
          } else {
            console.log('【调试】警告: resultUrls为空或不存在');
          }
        } catch (e) {
          console.error('【调试】解析resultJson失败:', e);
          console.error('【调试】原始resultJson:', data.resultJson);
        }
      } else if (data.state === 'failed') {
        status = 'failed';
        console.log('【调试】任务失败:', data.failMsg);
      } else {
        console.log('【调试】任务处理中, state:', data.state);
      }
      ...
      console.log('【调试】返回给前端的数据:', JSON.stringify(returnData, null, 2));
