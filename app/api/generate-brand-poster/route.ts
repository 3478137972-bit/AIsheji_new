...
const { generateBrandPosterPrompts } = require('@/lib/backend/deepseek')
...
export async function POST(request: NextRequest) {
  try {
    const { productName, sellingPoint, style, aspectRatio, referenceImages = [] } = await request.json()

    // 参数验证
    if (!productName || !sellingPoint) {
      return NextResponse.json(
        { success: false, error: '产品名称和卖点不能为空' },
        { status: 400 }
      )
    }

    console.log('收到品牌海报生成请求:', { productName, sellingPoint, style, aspectRatio, referenceImageCount: referenceImages.length })
    ...
    console.log('正在调用 DeepSeek 生成品牌海报提示词...')
    const promptResult = await generateBrandPosterPrompts({ productName, sellingPoint, style, aspectRatio })
