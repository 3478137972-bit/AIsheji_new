...
import { generateEventPosterPrompt } from '@/lib/api/deepseek'
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[活动海报] 步骤1: 调用 DeepSeek 生成提示词...')
    const prompt = await generateEventPosterPrompt({
      eventName: body.eventName,
      eventType: body.eventType,
      keyInfo: body.keyInfo,
      specialRequirements: body.specialRequirements,
    ...
    console.log('[活动海报] DeepSeek 生成的提示词:', prompt)

    // 步骤2: 上传参考图片(如果有)
    let imageInputUrls: string[] = []
    if (body.referenceImages && body.referenceImages.length > 0) {
      console.log(`[活动海报] 步骤2: 上传 ${body.referenceImages.length} 张参考图片...`)

      const uploadPromises = body.referenceImages.map((imageBase64, index) =>
        uploadImageToKIEAI(imageBase64, `event-poster-ref-${Date.now()}-${index}.jpg`)
      )

      const uploadResults = await Promise.all(uploadPromises)
      imageInputUrls = uploadResults.map((result) => result.downloadUrl)

      console.log('[活动海报] 参考图片上传成功:', imageInputUrls)
    } else {
      console.log('[活动海报] 步骤2: 无参考图片,跳过上传')
    }

    // 步骤3: 创建生成任务(生成1张图)
    const variantCount = 1
    console.log(`[活动海报] 步骤3: 创建 ${variantCount} 个生成任务...`)
    ...
    console.log('[活动海报] KIEAI 任务创建成功:', taskIds)
    ...
    console.log('[活动海报] 批次任务创建成功:', batchId)
